...
class GlanceNet(VAE):
    ...
    def __init__(self, args):
        ...
        print('Initialized GlanceNet model')
    ...
    def vae_classification(self, losses, x_true1, label1, y_true1, examples):
        ...
        # which examples do have concept supervision
        rn_mask = (examples==1)
        ...
        if n_passed > 0: # added the presence of only small labelled generative factors
            if self.latent_loss == 'MSE':                
                ...
                for i in range(label1.size(1)):
                    err_latent.append(nn.MSELoss(reduction='mean')(concepts[rn_mask][:, i], 2 * label1[rn_mask][:,i] - 1).detach().item() )
                ...
                loss_bin += nn.BCELoss(reduction='mean')((1+concepts[rn_mask][:, :label1.size(1)])/2,
                                                            label1[rn_mask] )
                ## track losses
                err_latent = []
                for i in range(label1.size(1)):
                    err_latent.append(nn.BCELoss(reduction='mean')((1+concepts[rn_mask][:, i])/2,
        ...
        return losses, {'x_recon': x_recon, 'mu': mu, 'z': z, 'logvar': logvar, 'latents': err_latent, 'n_passed': n_passed}
    ...
    def train(self, **kwargs):
        ...
        while not self.training_complete():
            ...
            # to evaluate performances on disentanglement
            z = np.zeros(len(self.data_loader.dataset), self.z_dim)
            g = np.zeros(len(self.data_loader.dataset), self.z_dim)
            
            for internal_iter, (x_true1, label1, _, examples) in enumerate(self.data_loader):
                ...
                # DSPRITES IS ENCODED DIFFERENTLY
                if self.dset_name == 'dsprites_full':
                    label1 = label1[:, 1:].to(self.device)
                else:
                    label1 = label1.to(self.device)
                ...
                ## ADD FOR EVALUATION PURPOSES
                z[internal_iter*self.batch_size:(internal_iter+1)*self.batch_size, :] = params['z'].detach().numpy()
                g[internal_iter*self.batch_size:(internal_iter+1)*self.batch_size, :label1.size(1)] = label1.detach().numpy()

                if (self.iter%self.show_loss)==0: 
                ...
                if track_changes and is_time_for(self.iter, self.test_iter):
                    ...
                    print('tracking training changes')
                    
                    self.train_call_losses(losses, params, 
                                            epoch,
                                            y_true1, label1, 
                                            Iterations, Epochs, Reconstructions, KLDs, True_Values, latent_errors, 
                ...
                if is_time_for(self.iter, self.test_iter):
                    ...
                    factors = pd.DataFrame(
                        {'iter': self.iter+1, 
                         'rec': trec, 'kld': tkld, 'latent': tlat, 'BCE': tbce, 
                         'Acc': tacc, 'I': I_tot}, index=[0])
    ...
    def train_call_losses(self, losses, params, 
                          epoch,
                          y_true1, label1, 
                          Iterations, Epochs, Reconstructions, KLDs, True_Values, latent_errors, 
                          Accuracies, F1_scores,
        ...
        F1_scores.append(
                            Accuracy_Loss()(params['prediction'], y_true1, dims=self.n_classes).item()
                        )
